...
object IntCodes:
  ...
  val getTape =
    (sourceHead map splitCsv andThen parseLongs).filterOrDieMessage(_.nonEmpty)("Empty tape").map(IArray(_*))
  ...
  def validate(mods: Array[Long], op: Long)(using State): Option[Suspend | State] =
    Codes
      .get(op)
      .flatMap(op => mods.flatMap(Modes.get).cond(_.size == 3)(as => op(using (as(2)(1), as(1)(2), as(0)(3)))))
  ...
  def initial(init: IArray[Long], in: Int*) = State(init, LongMap.empty, 0, 0L, LazyList(in*).map(_.toLong), Nil)
