...
import java.lang.Math.{PI, abs, atan2, max, toDegrees}
...
object Day10:
  ...
  def collectTrue(coords: Coords): State =
    (0 until coords.length).foldLeft(List.empty[Coord])((acc, x) =>
      (0 until coords(0).length).foldLeft(acc)((acc, y) => if coords(x)(y) then Coord(x, y) :: acc else acc)
    )
  ...
  val mostFound = map flatMap (collectTrue andThen (searchAll(_) map (_._3)))
  ...
  def searchThenMost(index: Int) =
    map map collectTrue flatMap (s => searchAll(s) map (p => mostAt(p._1, p._2)(s).toSeq.sortBy(_._1).apply(index)._2))
